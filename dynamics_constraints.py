...
def CollocationConstraintEvaluator(planar_arm, context, dt, x_i, u_i, x_ip1, u_ip1):
  ...
  dynamics_current=EvaluateDynamics(planar_arm,context,x_i,u_i)
  dynamics_next=EvaluateDynamics(planar_arm,context,x_ip1,u_ip1)
  state =0.5*(x_i+x_ip1)- dt*(dynamics_next-dynamics_current)*0.125
  control = 0.5*(u_i+u_ip1)
  dynamics_eval = EvaluateDynamics(planar_arm,context,state,control)
  h_i = 1.5*(x_ip1-x_i)/dt - 0.25*(dynamics_next+dynamics_current) - dynamics_eval
  return h_i
...
def AddCollocationConstraints(prog, planar_arm, context, N, x, u, timesteps):
  ...
  for i in range(N - 1):
    ...
    lb=np.zeros(n_x)
    ub=lb
    prog.AddConstraint(CollocationConstraintHelper,lb,ub,np.hstack((x[i],u[i],x[i+1],u[i+1])))